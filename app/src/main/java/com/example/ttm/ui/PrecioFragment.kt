...
import com.example.ttm.databinding.FragmentPrecioBinding
...
class PrecioFragment : Fragment() {
    ...
    private var binding: FragmentPrecioBinding? = null

    override fun onCreateView(
            inflater: LayoutInflater, container: ViewGroup?,
            savedInstanceState: Bundle?
    ): View? {
        binding = FragmentPrecioBinding.inflate(inflater, container, false)
        val binding = binding!!
        val view = binding.root

        lifecycleScope.launch{
            val list = withContext(Dispatchers.IO) {
                App.obtenerDB().terminalesDao().findOneByPrecio()
            }
