...
import com.example.ttm.databinding.FragmentPixelesBinding
import com.example.ttm.models.Producto
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext

class PixelesFragment : Fragment() {

    private var listener: MainListener? = null
    private var binding: FragmentPixelesBinding? = null

    override fun onCreateView(
            inflater: LayoutInflater, container: ViewGroup?,
            savedInstanceState: Bundle?
    ): View? {
        binding = FragmentPixelesBinding.inflate(inflater, container, false)
        val binding = binding!!
        val view = binding.root

        lifecycleScope.launch {
            val list = withContext(Dispatchers.IO) {
                App.obtenerDB().terminalesDao().findOneByPixeles()
            }
